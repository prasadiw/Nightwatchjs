...
module.exports = {
	...
	'Test Auto Complete Search with TAGS': function (browser) {
		browser
			.waitForElementVisible('body', 1000)
			.waitForElementVisible('#global-search-container', 5000)
			...
			.waitForElementVisible('#global-search-container > div.search-container.search-focused > div.global-search-autocomplete', 10000)
			.pause(2000)
				...
				if (result.status != -1) { //wait for auto suggestions using Users
					...
					browser.element('css selector', '#captions > div.captions-container.infinite-scroll-loaded', function (result) {
						if (result.status != -1) { //If user comments results are found, locate the first user comment
							browser.waitForElementVisible('#captions > div.captions-container.infinite-scroll-loaded > div > div > div:nth-child(1) > div.comment-wrapper', 5000)
